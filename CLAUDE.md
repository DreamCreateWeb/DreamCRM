...
| Website | Website (workspace) | `/website` | ONE sidebar entry → the Shopify-style hub: live-site card (real URL + domain-state pill + last edit), go-live checklist (real states only), 30-day performance snapshot, doorway cards w/ live stats + honest below-plan upsell cards. Sub-pages: `/website/editor` (the full-screen Studio — EditBridge, section modals, AI bar, page navigator, 🎨 Design picker; top bar slimmed to editing controls), `/website/blog` (was `/posts`; platform org authors the marketing blog through it too), `/website/seo` (was `/seo`; absorbed the `/settings/seo` Search-appearance meta editor at `#meta`), `/website/careers` (was `/careers`; Premium; ATS + JSON-LD), `/website/domain` (the auto-polling custom-domain card, ex-`/settings/clinic` anchor), `/website/share` (QR cards). Old paths 308 via route-level stubs (NEVER next.config — it would hijack public clinic-site paths pre-middleware). `/settings/clinic` remains the deep-edit fallback. |
...
| Settings | Settings | `/settings` | Card-grid home → 13 focused pages (clinic, practice, locations, portal, automations/emails, message-templates, team, apps, billing, account, notifications, security, feedback) + 3 redirect stubs (plans, reminders, seo → /website/seo#meta). |
