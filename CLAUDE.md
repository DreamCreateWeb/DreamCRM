...
| Business | Shop | `/shop` | **Live (v1 — slice 2 of N)** | Premium-tier. Phase 3 differentiator (no orbital-layer competitor ships a storefront — confirmed Weave/NexHealth/RevenueWell have none). Built in slices: **(1 shipped)** migration 0032 = 8 purpose-built `shop_*`/`membership*` tables (separate from the generic Mosaic products/orders), Connect *Standard* designed so payouts land in the clinic's own bank. **(2 shipped)** `/shop` admin: product/variant catalog CRUD (`/shop/products/new` + `/shop/products/[id]`, image upload to S3, multi-variant pricing + inventory, FSA-with-Rx flag, draft/active/archived), fulfillment + tax config toggles, Stripe Connect status card (inert until `STRIPE_CONNECT_CLIENT_ID` is set). Client-safe types/labels in `lib/types/shop.ts`; DB in `lib/services/shop.ts`. Demo seeder: 6 products (7 variants) across categories/statuses + config; self-heal backfills legacy demos. **Next slices:** (3) public storefront `/site/[slug]/shop` + cart + Stripe Connect checkout (pickup+ship+Stripe Tax) + orders admin + patient-linked orders; (4) birthday/loyalty coupons; (5) membership plans (recurring cash-pay, $30-45/mo, drives 76% more visits — `membership_plan`/`membership` tables ready). **Research-grounded:** FSA/HSA is mostly a myth (cosmetic whitening + plain brushes ineligible; electric brushes only with an Rx) so it's an optional per-product flag, not a headline. **Stripe Connect can't be fully sandbox-tested** (no connected accounts/cards) — logic is unit-tested; money flow verified in Stripe test mode |
