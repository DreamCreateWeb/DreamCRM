...
| [`BRAND.md`](./BRAND.md) | The MARKETING SITE's personality language ("Daylight Dream", owner-approved 2026-09-14 on DREAMCRM-43 and reversed from a night hero to light throughout on DREAMCRM-67, 2026-09-15) — mood, palette energy, shape, type, emoji/copy voice, motion + the cinematic scroll spine, the contrast law, scaling and imagery. NOTE: **THE WHOLE MARKETING SITE LOOKS LIKE IT — the Part 8 build order is COMPLETE (DREAMCRM-80, 2026-09-16).** Move 1 landed on DREAMCRM-69 (the night band is deleted, not dormant), move 2 on DREAMCRM-70 (the cinematic pin-and-scroll section under the ticker), move 3 on DREAMCRM-71 (tone tiles replace `CheckIcon` everywhere) and move 4 on DREAMCRM-72 (the shared chrome — `MarketingHeader`, `MarketingFooter`, `PageHero` — so all eight subpages now OPEN in the language). Both owner vetoes are closed in code at every call site: `NIGHT_GRID`, `HERO_DOT_GRID` and `CheckIcon` are deleted rather than dormant, and `tests/marketing/no-drawn-grid.test.ts` + `tests/marketing/tone-tiles.test.ts` hold each at zero by asking the tree. Move 5 landed on DREAMCRM-73 (`scripts/decorative-layer-grade.mjs` — the renamed decorative-layer grader, re-anchored at the light hero, the ticker and the final CTA panel, extremum per sample rather than a global constant; measured run in Part 7). Move 6 converted every page BODY, one page per PR: PRICING landed on DREAMCRM-75 (hard-left sections on one column, the price panel breaking the hero's seam, Part 3's radius ladder, the 26-row inventory's two-tier shape kept deliberately, and every price on the page resolved from `getQuotedPlan()` — the grader now takes a `path` per sample and grades subpages too). COMPARE landed on DREAMCRM-76 (both files; `compare/[vendor]` had never inherited move 4's `PageHero` and gained it, plus a two-price panel breaking the seam; `SectionOpener` promoted out of the pricing page into `components/marketing/ui.tsx`) — and it CLOSED the two Part 5 ledger entries on the capability matrix by reflowing it (card per capability below `md`, a real `<table>` with no `min-width` above), so `/compare/[vendor]` no longer drags 212px sideways at 390 and has no scroll region for `scrollable-region-focusable` to be about. The cause was NOT the "flex/grid item without `min-width: 0`" the ledger guessed: `MatrixMark`'s `sr-only` spans are `position: absolute` with no positioned ancestor inside the scroll box, so their containing block resolved to the ICB outside it and they widened the DOCUMENT from their static positions — scrollable overflow follows the containing-block chain, not DOM ancestry. Part 10's zero-horizontal-scroll rule is now a TEST rather than a habit: `e2e/marketing-viewport.spec.ts` grades every code-backed marketing page at 390/834/1440 on `documentElement.scrollWidth` (a new blocking-assertion class inside `e2e` — registered in `INTAKE_RULES`). PRODUCT landed on DREAMCRM-77 (the longest page on the site; the move was STRUCTURE rather than skin — it is a numbered tour now, with a chapter mark per section and a sticky CHAPTER RAIL carrying the header's own gradient underline on the chapter you are in, hard left on one column with the left/right alternation retired, the product column widened from six grid columns to seven). Two things from it generalise: a sticky structural rail column is a narrower CONTAINER by another name (the first draft moved every heading ~180px right of the hero's), and the page's translucent sticky nav went OPAQUE rather than gaining a second guarded alpha — `token-contrast.test.ts`'s sticky-fill arithmetic reads `chrome.tsx` by name and asserts exactly one alpha there, so a second glass bar anywhere else on the site is ungraded by construction. WHY landed on DREAMCRM-78 (the manifesto; the six beliefs stop being a card grid and become a numbered manifesto — claim left, argument right, `NN / 06` as a mono numeral, hard left on the hero's own column; nothing below the hero carries a decorative layer, which is Part 7's answer for a reading page; the two checkable honesty tenets carry a receipt link to the page that proves them and THE PRICE IS ON THE PAGE, resolved from `getQuotedPlan()` — the pricing-price-source scan now covers this route too, and all three tenets are pinned against a restyle that softens a body rather than a heading). RESOURCES landed on DREAMCRM-79 (four routes, not one — the hub plus three guides; the three ARTICLES had never inherited move 4 at all, they rendered a bespoke `from-teal-50/60` band, which is the SECOND time the count of pages wearing this brand was wrong in the same direction after `compare/[vendor]`. The hub's three equal cards became an INDEX whose rows say what is actually in each guide; the articles gained a CHAPTER RAIL derived from their own `GuideH2` children, on the RIGHT so the reading column still starts at the `h1` — page 3's rail lesson with the sign flipped; `ScriptCard` grew `kind="figures"` because the membership guide's leader-dot arithmetic had never lined up in a proportional face under `pre-line`, which also ate its indentation; and the shared shell's typed `$200/mo` now resolves from `getQuotedPlan()` — it was live on three public pages while counting as one surface. Two new axe stops at a ceiling of ZERO. No emoji: the ISSUE asked for them and was wrong — Part 5's body-copy clause is for plain unicode AND needs a named human, and these guides have no byline). PAGE 6 CLOSED THE MOVE on DREAMCRM-80 — the six routes the build order never named, in two PRs: the three TOOLS (`/grade`, `/roi`, `/partner-program`) and the three CONTENT routes (`/blog`, `/changelog`, `/docs`), eight files once the dynamic halves are counted. Five of the six had a BESPOKE hero — `/grade` had no hero component at all, `/roi` and `/partner-program` wore the same `from-teal-50/60` band `compare/[vendor]` and `GuideShell` were found in, and `/docs/[slug]` and `/blog/[slug]` had none, which the ISSUE ITSELF had listed as already converted. That is the THIRD and FOURTH time the count of converted pages was wrong in the same direction, so the rule widens: an audit written by reading ROUTE FOLDERS is unreliable whoever wrote it, and the instrument that has never been wrong is `git grep PageHero -- "app/(marketing)"`. The three tools are INSTRUMENTS and nothing in them touches what they compute — `computeRecallRoi`, the grader's checks and the three hedged scenarios are untouched; what changed is Part 4's mono register on every figure meant to be compared, plus a computed mono equation on `/roi` and `/partner-program` in place of arithmetic spelled out in prose. THE LAST THREE DECORATIVE EMOJI ON THE SITE ARE GONE (`/grade`'s three icon-emoji, `/roi`'s padlock) and ONE ARRIVED, at `planet`'s first ever call site — the empty `/blog`, the one empty state on the marketing site. THE CHANGELOG'S KIND PILLS WERE A PART 2 VIOLATION nobody had noticed: `teal` is the brand ramp and "the brand hue is never a status", `blue` is not in the tone registry at all, so the kind became a mono word and the COLOUR MOVED TO THE SUBJECT (`ChangelogItem.glyph`, required) — the weekly cadence is untouched. Part 11's blog-cover LAYOUT-SHIFT defect closed in passing (`width`/`height` + a matching `aspect-[1200/630]`; the fix is the RATIO, not the intrinsic size). The price-provenance scan now covers NINE surfaces, and the ninth is a new shape: `lib/recall-roi.ts` held `PLAN_PRICE_MONTHLY = 200` and `computeRecallRoi` DIVIDES by it, so a reprice would have made `/roi`'s break-even line arithmetically WRONG rather than merely stale — a price that is an INPUT TO A CALCULATION is the shape to look for next. SIX NEW AXE STOPS at a ceiling of ZERO (eleven marketing stops total; only `/product` and `/compare` lack one, and both are named in `e2e/smoke.spec.ts`). `FaqList` promoted out of `/pricing` into `components/marketing/ui.tsx` — the `SectionOpener` move, at the fourth call site. THE ONE HONEST GAP: `/blog` and `/blog/[slug]` are converted but UNGRADED — their bodies come from the database and both browser suites are seed-free, so they were measured by hand in both states at all three widths and a seeded stop is the follow-up. Two rules page 5 changed for everyone still hold: GRADE ALL THREE WIDTHS — page 5's worst runs were at 390 and page 6's are almost all at 834, which is the same rule (the extremum follows the RUN, not the page) rather than a contradiction — and CHECK WHETHER A PAGE ACTUALLY RENDERS `PageHero`. Part 8 records what each move settled. The hand-graded contrast table still binds the FOOTER, which stays dark (no automated guard can see a dark band inside a light-mode page). Read before touching `app/(marketing)` or `components/marketing`. Scope: marketing only — the dashboard keeps DESIGN-SYSTEM.md. |
