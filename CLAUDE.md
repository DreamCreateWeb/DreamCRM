...
| [`BRAND.md`](./BRAND.md) | The MARKETING SITE's personality language ("Daylight Dream", owner-approved 2026-09-14 on DREAMCRM-43 and reversed from a night hero to light throughout on DREAMCRM-67, 2026-09-15) — mood, palette energy, shape, type, emoji/copy voice, motion + the cinematic scroll spine, the contrast law, scaling and imagery. NOTE: the FOLD and the scroll spine look like it — Part 8 move 1 landed on DREAMCRM-69 (the night band is deleted, not dormant) and move 2 landed on DREAMCRM-70 (the cinematic pin-and-scroll section under the ticker) — and move 3 landed on DREAMCRM-71 (the tone tiles replace `CheckIcon` everywhere on the marketing site — both owner vetoes are now closed in code, deleted rather than dormant); moves 4–6 are ahead, so the shared chrome and every subpage LAYOUT still wear the old language. Part 8 says which move closes each gap. The hand-graded contrast table still binds the FOOTER, which stays dark (no automated guard can see a dark band inside a light-mode page). Read before touching `app/(marketing)` or `components/marketing`. Scope: marketing only — the dashboard keeps DESIGN-SYSTEM.md. |
