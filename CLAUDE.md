...
- **PMS Integrations v1 (Open Dental, two-way)** — the orbital layer
  ...
  `lib/types/pms.ts`. **Can't be tested against a live Open Dental** (no
  instance + the $30/mo office fee) — engine + adapter unit-tested with
  a mocked `fetch`; the demo provider exercises the engine end-to-end.
  Demo seeder pump: a sandbox "Open Dental (Sandbox)" connection +
