...
| [`BRAND.md`](./BRAND.md) | The MARKETING SITE's personality language ("Daylight Dream", owner-approved 2026-09-14 on DREAMCRM-43 and reversed from a night hero to light throughout on DREAMCRM-67, 2026-09-15) — mood, palette energy, shape, type, emoji/copy voice, motion + the cinematic scroll spine, the contrast law, scaling and imagery. NOTE: the site does not look like it yet — BRAND.md Part 8 is the build order, and the homepage still renders the retired night band. The hand-graded contrast table still binds the FOOTER, which stays dark (no automated guard can see a dark band inside a light-mode page). Read before touching `app/(marketing)` or `components/marketing`. Scope: marketing only — the dashboard keeps DESIGN-SYSTEM.md. |
