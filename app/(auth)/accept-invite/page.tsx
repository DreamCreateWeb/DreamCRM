...
function AcceptInviteInner() {
  ...
  async function acceptNow(orgName: string) {
    ...
    if (result.error) {
      setStep({ type: 'error', message: result.error.message ?? 'Could not accept the invitation.' })
      return
    }
  ...
  async function handleCreateAccount(e: React.FormEvent<HTMLFormElement>) {
    ...
    // Sign up with just email + name + password — no org creation
    ...
    // Account created + auto-signed-in → accept invitation
    setStep({ type: 'accepting' })
        ...
        <form onSubmit={isSignIn ? handleSignIn : handleCreateAccount} className="space-y-4">
          ...
          >
            {submitting
              ...
              : (isSignIn ? 'Sign In & Accept Invite' : 'Create Account & Accept Invite')}
          </button>
        </form>
        ...
        <div className="mt-5 pt-5 border-t border-gray-100 dark:border-gray-700/60 text-sm text-gray-500">
          {isSignIn ? (
            ...
            <>
              ...
              <button
                onClick={() => { setAuthMode('signin'); setFormError(''); setPassword('') }}
                className="font-medium text-violet-500 hover:text-violet-600"
              >
                Sign in instead
              </button>
