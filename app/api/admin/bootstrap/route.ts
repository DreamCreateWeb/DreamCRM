...
/**
 * One-time migration runner, gated by ADMIN_BOOTSTRAP_TOKEN.
 *
 * Removed after each use — rotate the token and delete this route as part
 * of the cleanup PR. Do NOT leave it in main.
 *
 * Usage:
...
const TOLERABLE_PG_CODES = new Set(['42P07', '42701', '42710']) // already exists variants
...
async function runMigrationFile(filename: string, sqlText: string): Promise<{ ok: true; skipped: number } | { ok: false; error: string }> {
  ...
  for (const stmt of statements) {
    try {
      ...
      if (code && TOLERABLE_PG_CODES.has(code)) {
        skipped++
        continue
      }
...
export async function POST(req: NextRequest) {
  ...
  if (!expected) {
    return NextResponse.json({ error: 'ADMIN_BOOTSTRAP_TOKEN is not set' }, { status: 503 })
  }
  ...
  for (const filename of files) {
    if (applied.has(filename)) {
      results.push({ filename, status: 'already-applied' })
      continue
    }
