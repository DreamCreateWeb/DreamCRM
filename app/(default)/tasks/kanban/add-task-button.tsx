...
export default function AddTaskButton({
  ...
  function onSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await addTask({ title, description: description || null, status, priority })
        setTitle('')
        setDescription('')
        setOpen(false)
      } catch (err) {
            ...
            <TransitionChild as={Fragment} enter="ease-out duration-200" enterFrom="opacity-0 scale-95" enterTo="opacity-100 scale-100" leave="ease-in duration-150" leaveFrom="opacity-100 scale-100" leaveTo="opacity-0 scale-95">
              <DialogPanel className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full">
                <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700/60">
                  <h2 className="font-semibold text-gray-800 dark:text-gray-100">Add Task</h2>
                </div>
                  ...
                  <div className="px-5 py-4 space-y-4">
                    ...
                    <div className="flex space-x-3">
                      <div className="flex-1">
                        ...
                        <select className="form-select w-full" value={status} onChange={(e) => setStatus(e.target.value as TaskStatus)}>
                          {TASK_STATUSES.map((s) => (
                      ...
                      <div className="flex-1">
                        ...
                        <select className="form-select w-full" value={priority} onChange={(e) => setPriority(e.target.value as any)}>
