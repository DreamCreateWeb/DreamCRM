 ...
 *
 *    It is belt AND braces: the CSS in `MarketingMotionStyles` ALSO hard-
 *    reverts `.is-cinematic` under `prefers-reduced-motion`, a coarse pointer
 *    and any viewport under `lg`. Either half alone is sufficient; the reason
 *    for both is that the JS half is the one that can be wrong about the
 *    environment, and the CSS half cannot.
 *
...
export default function CinematicSpine() {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      const on = legal()
      if (on === root.classList.contains('is-cinematic')) {
        if (on) paint()
        return
      }
      root.classList.toggle('is-cinematic', on)
      if (on) {
        paint()
      } else {
