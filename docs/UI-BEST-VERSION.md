...
- **214 WCAG AA violations were live on `main` the whole time, in three
  ...
  and of those 79, 41 were the WCAG-incidental decorative mocks described
  below, so 38 are genuine. **QA has since ruled those 41 EXEMPT rather than
  carried** (`A11Y_INCIDENTAL` in `e2e/axe-baseline.ts`, 2026-09-11): the
  subtrees leave the scan, `marketing: home`'s entry is deleted rather than
  lowered, and the stop is back to zero tolerance. **The baseline now reads
  38, all `color-contrast`, and every number in it is a real defect.**
  `nested-interactive` and `list` are CLOSED entirely; the baseline is a single
  rule. The file carries a running burn-down log. One correction to the framing above,
  ...
  And 41 of `marketing: home`'s 45 original instances are inside `aria-hidden`
  decorative product mock-ups at 7.7–10.9px, which WCAG 1.4.3 exempts as
  incidental; they are deliberately not "fixed", and are now exempt from the
  scan rather than sitting in it as a ceiling that could never come down. (The
  other 4 were the marketing footer's column headings, fixed in batch 55.) Remaining: white text on the
