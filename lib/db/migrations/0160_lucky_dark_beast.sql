...
-- exactly the duplicates this migration exists to prevent. Scoped to the
-- automated touches the new index covers (`sent_by_user_id is null and
-- template is not null`); staff drawer sends and ad-hoc NULL-template rows
-- are untouched.
--
...
USING (
	...
	WHERE "sent_by_user_id" IS NULL AND "template" IS NOT NULL
) dup
WHERE a."id" = dup."id" AND dup.rn > 1;--> statement-breakpoint
-- STEP 2 (drizzle-generated): the guard itself.
CREATE UNIQUE INDEX "appt_reminder_auto_touch_uq" ON "appointment_reminder_log" USING btree ("appointment_id","template") WHERE "appointment_reminder_log"."sent_by_user_id" is null and "appointment_reminder_log"."template" is not null;
