...
/**
 * Returns the 30-min slot grid for a given day at the given clinic. Slots
 * before "now" are filtered out so a patient never sees a past-time option.
 *
 * `excludeAppointmentId` lets a future "reschedule" flow ignore the
 * appointment being moved so its current slot still appears as available.
 */
export async function getAvailableSlots(
  ...
  if (!openParts || !closeParts) return []
