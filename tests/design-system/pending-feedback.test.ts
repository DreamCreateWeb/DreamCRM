...
/**
 * `{cond ? 'Saving…' : <anything>}` — a JSX child whose BUSY arm is a quoted
 * string carrying an ellipsis. The idle arm is deliberately unconstrained.
 *
 * WIDENED IN BATCH 60. The original required BOTH arms to be single-quoted
 * literals, and four live offenders sat in the gap — every one of them with a
 * template literal or a nested ternary on the idle arm, which is what you
 * write the moment the label interpolates anything:
 *
 *   {pending ? 'Importing…' : `Import ${n} patients`}
 *   {pending ? 'Working…'   : `Add more — $${price}/mo`}
 *   {isPending ? 'Redirecting…' : onTrial ? `Choose ${p}` : `Switch to ${p}`}
 *
 * The busy arm is the part that identifies the shape, and it is the part that
 * was always a plain string. Matching on that alone is both narrower in what
 * it assumes and wider in what it catches.
 */
const LABEL_TERNARY = /\{\s*([^{}?]{1,80}?)\s*\?\s*'([^']*(?:…|\.\.\.)[^']*)'\s*:\s*([^}]{1,120}?)\s*\}/g
 ...
 *
 * 71 → 90 IN BATCH 60, AND THAT IS NOT A REGRESSION — no raw button gained a
 * ternary. `LABEL_TERNARY` widened (see above), so the same population is
 * being measured with an instrument that can finally see all of it: 19 of
 * these were always there and always invisible, because their idle arm is a
