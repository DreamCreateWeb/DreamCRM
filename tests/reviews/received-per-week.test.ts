...
/**
 * getReviewsReceivedPerWeek8 — the Growth hub's Reviews-door heartbeat series
 * (v3 law 7). Mirrors tests/payments/collected-per-week.test.ts; two things
 * are non-negotiable and pinned here:
 *   1. ORG SCOPING — the query filters by organizationId (ORG_A sees its
 *      reviews, never ORG_B's). The db mock filters rows by whichever org id
 ...
 *      clinicWeekStart against the clinic tz. A Saturday-9pm-Central review
 *      is already Sunday in UTC — it must land in the PREVIOUS clinic-local
 *      week, not the new one.
 * Plus the timestamp-field contract: bucketing rides `reviewCreatedAt` (when
 * the review was posted on the platform), never our sync-insert `createdAt`.
 */
...
import { getReviewsReceivedPerWeek8 } from '@/lib/services/google-reviews'
...
describe('getReviewsReceivedPerWeek8', () => {
  it('returns 8 clinic-local weeks, oldest first, labeled by week-start date', async () => {
    const series = await getReviewsReceivedPerWeek8('org_a', NOW)
  ...
  it('is org-scoped: ORG_A counts its reviews, never ORG_B’s', async () => {
    state.rows = [
      { organizationId: 'org_a', reviewCreatedAt: new Date('2026-01-05T15:00:00.000Z') },
      { organizationId: 'org_b', reviewCreatedAt: new Date('2026-01-05T15:00:00.000Z') },
      { organizationId: 'org_b', reviewCreatedAt: new Date('2026-01-05T16:00:00.000Z') },
    ]
    const a = await getReviewsReceivedPerWeek8('org_a', NOW)
    ...
    const b = await getReviewsReceivedPerWeek8('org_b', NOW)
    expect(b.reduce((s, p) => s + p.value, 0)).toBe(2)
  })

  it('buckets a UTC-Sunday / clinic-Saturday review into the PREVIOUS clinic-local week', async () => {
    state.rows = [
      ...
      { organizationId: 'org_a', reviewCreatedAt: new Date('2026-01-04T02:00:00.000Z') },
      // Exactly the clinic-local week boundary (Sun Jan 4 00:00 CST) —
      // inclusive start of the new week.
      { organizationId: 'org_a', reviewCreatedAt: new Date('2026-01-04T06:00:00.000Z') },
      // A row the platform shipped without a created date never counts.
      { organizationId: 'org_a', reviewCreatedAt: null },
    ]
    const series = await getReviewsReceivedPerWeek8('org_a', NOW)
    const byBucket = Object.fromEntries(series.map((p) => [p.bucket, p.value]))
    expect(byBucket['Dec 28']).toBe(1) // the Saturday-night review stayed put
    expect(byBucket['Jan 4']).toBe(1)
  })
})
